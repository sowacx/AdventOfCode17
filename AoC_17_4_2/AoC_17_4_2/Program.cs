...
namespace AoC_17_4_2
{
    class Program
    {
        static void Main(string[] args)
        {
            string input = @"nyot babgr babgr kqtu kqtu kzshonp ylyk psqk
    ...
    public static class PuzzleSolver
    {
        public static int Solve(string input)
        {
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                List<string> rearangedWords = new List<string>();
                for (int j = 0; j < words.Length; j++)
                {
                    rearangedWords.Add(String.Concat(words[j].OrderBy(c => c)));
                }

                bool isRepeating = false;
                List<string> noDuplicatesList = rearangedWords.Distinct().ToList();
                if (rearangedWords.Count != noDuplicatesList.Count)
                {
                    isRepeating = true;
                }
                distinctLines += isRepeating ? 0 : 1;
            } 
